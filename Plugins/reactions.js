...
module.exports = {
  ...
  start: async (Atlas, m, { text, prefix, mentionByTag, doReact }) => {
    try {
      ...
      const userResponse = single ? "Themselves" : `@${reactant.split("@")[0]}`;
      const txtResponse = suitableWords[reaction];
      const caption = `*@${m.sender.split("@")[0]} ${txtResponse} ${userResponse} like a pro!*`;
