...
if (text.endsWith("--info") || text.endsWith("--i") || text.endsWith("--?")) {
  ...
  return Atlas.sendMessage(
    ...
    if (m) {
  let userID = botNumber;
  let botName = packname;
  let newBotState = "online"   // Set the bot state based on the condition

 const hasUp = await updateBot(userID, botName, newBotState);
  //const hasUp = await setBotOnline(userID, botName, newBotState);
}
